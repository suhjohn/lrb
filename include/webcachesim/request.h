...
class SimpleRequest
        ...
        SimpleRequest(KeyT id, uint64_t size, uint64_t t, uint64_t real_time_t, vector<uint16_t> *extra_features = nullptr)
        : _id(id), _size(size), _t(t), _real_time_t(real_time_t) {
            if (extra_features)
        ...
        inline void reinit(KeyT id, uint64_t size, uint64_t t, uint64_t real_time_t, vector<uint16_t> *extra_features = nullptr) {
            _id = id;
            _size = size;
            _t = t;
            _real_time_t = real_time_t;
            if (extra_features)
                _extra_features = *extra_features;
        }
